...
function checkMacro() {
  fs.exists('macro', function(exists) { 
    if(exists) {
      getIntervals();
    } else {
      fs.mkdir('macro', function(e) {
        getIntervals();
      });
    }
  });
...
function getJSON() {
  var url = 'http://strata.geology.wisc.edu/readonly/getmap/geojson_macro3.php?interval_name=' + intervals[i];
  ...
  http.get(url, function(res) {
      ...
      res.on('end', function() {
          ...
          fs.writeFile("tempjson/macro_" + filename + ".json", JSON.stringify(response), function(err) {
              if(err) {
                  console.log(err);
              } else {
                  ...
                  exec('topojson -o macro/macro_' + filename +'.json -p col_id,lith_type,p -- tempjson/macro_' + filename + '.json', function(err, result) {
                    if (err) {
                      ...
                      fs.unlink('tempjson/macro_' + filename + '.json');
